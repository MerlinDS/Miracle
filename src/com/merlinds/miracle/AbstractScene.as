...
class AssetsParser
{
	...
	private var _mtfSerializer:MTFSerializer;
	private var _mafSerializer:MAFSerializer;
	...
	public function execute(callback:Function, scale:Number):void
	{
		_callback = callback;
		_scale = scale;
		_mtfSerializer = new MTFSerializer( true );
		_mafSerializer = new MAFSerializer( true );
		trace("Miracle: Start serialization");
		this.parseAssets();
	}
	...
	private function parseAssets():void
	{
		if ( _assets.length > 0 )
		{
			...
			{
				_mtfSerializer.deserialize( _currentAsset.output, _meshes,
						_scale, _currentAsset.name, deserializationComplete );
			}
			else if( _currentAsset.type == Asset.TIMELINE_TYPE)
			{
				_mafSerializer.deserialize( _currentAsset.output, _animations,
						_scale, _currentAsset.name, deserializationComplete );
			}
		...
		{
			trace("Miracle: End serialization");
			this.complete();
		}
	}
	
	private function deserializationComplete():void
	{
		_currentAsset.destroy();
		this.parseAssets();
	}
